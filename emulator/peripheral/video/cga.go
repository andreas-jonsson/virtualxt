...
func (m *CGADevice) Install(p processor.Processor) error {
	m.p = p
	m.windowTitleTicker = time.NewTicker(time.Second)
	m.quitChan = make(chan struct{})

	m.mem = make([]byte, memorySize)
...
func (m *CGADevice) Name() string {
	return "CGA/HGC Compatible Device"
}

func (m *CGADevice) Reset() {
	m.lock.Lock()
...
func (m *CGADevice) setBlit(b bool) {
	m.lock.Lock()
	m.enableBlit = b
	m.lock.Unlock()
}

func (m *CGADevice) Step(cycles int) error {
...
func (m *CGADevice) blitChar(ch, attrib byte, x, y int) {
	pixels := m.surface
...
func (m *CGADevice) updateWindowTitle() {
	select {
	case <-m.windowTitleTicker.C:
		...
		platform.Instance.SetTitle(fmt.Sprintf("VirtualXT - %.2f MIPS%s", numCycles/1000000, hlp))
	default:
	}
}

func (m *CGADevice) renderLoop() {
	p := platform.Instance
...
func (m *CGADevice) In(port uint16) byte {
	m.lock.RLock()
...
func (m *CGADevice) Out(port uint16, data byte) {
	m.lock.Lock()
...
func (m *CGADevice) ReadByte(addr memory.Pointer) byte {
	m.lock.RLock()
...
func (m *CGADevice) WriteByte(addr memory.Pointer, data byte) {
	m.lock.Lock()
	atomic.StoreInt32(&m.dirtyMemory, 1)
